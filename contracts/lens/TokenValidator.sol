...
contract TokenValidator is ITokenValidator, IPegasysCallee, ImmutableState {
    ...
    function _validate(address token, address baseToken, uint256 amountToBorrow) internal returns (Status) {
        ...
        address pairAddress = PegasysLibrary.pairFor(this.factoryV2(), token, baseToken);
        ...
        (, bytes memory returnData) = address(pairAddress).call(abi.encodeWithSelector(IPegasysPair.token0.selector));
        ...
        IPegasysPair pair = IPegasysPair(pairAddress);
    ...
    function pegasysCall(address, uint256 amount0, uint256, bytes calldata data) external view override {
        IPegasysPair pair = IPegasysPair(msg.sender);
